/*
 *	InfoNES Android Port by Scott Glover
...
long run_emulator(char *rom_path)
{
  ...
  const SDL_VideoInfo* cvid = SDL_GetVideoInfo();
  screen_w = cvid->current_w;
  screen_h = cvid->current_h;
  fprintf(stderr, "Current video resolution: X: %d Y: %d\n", screen_w, screen_h);

  vmode = SDL_HWPALETTE | SDL_SWSURFACE | SDL_DOUBLEBUF | SDL_FULLSCREEN;
  screen = SDL_SetVideoMode(screen_w, screen_h, 8, vmode);
